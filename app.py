...
def api_reset_set(set_id):
    db = get_db()
    row = db.execute(
        """SELECT s.id, m.id AS mid, m.completed, m.season_id FROM sets s
           JOIN games g ON g.id=s.game_id JOIN matches m ON m.id=g.match_id
           WHERE s.id=?""", (set_id,)).fetchone()
    if not row:
        return _err("Set not found", 404)
    if row["completed"]:
        return _err("Match is completed. Reopen it before resetting a set.")
...
@app.post("/api/set/<int:set_id>/move")
@admin_required
def api_move_set(set_id):
    """Move (or swap) a set's throwers and throws into another set slot of
    the SAME match."""
    db = get_db()
    data = request.get_json(silent=True) or {}
    try:
        target_id = int(data.get("target_set_id"))
    except (TypeError, ValueError):
        return _err("Pick a target set.")
    if target_id == set_id:
        return _err("That's the same set.")
    def info(sid_):
        return db.execute(
            """SELECT s.*, g.match_id AS mid, m.completed, m.season_id
               FROM sets s JOIN games g ON g.id=s.game_id
               JOIN matches m ON m.id=g.match_id WHERE s.id=?""",
            (sid_,)).fetchone()
    src_, dst = info(set_id), info(target_id)
    if not src_ or not dst:
        return _err("Set not found", 404)
    if src_["mid"] != dst["mid"]:
        return _err("Both sets must belong to the same match.")
    if src_["completed"]:
        return _err("Match is completed. Reopen it before moving sets.")
    # capture, wipe, and re-place both sets' contents (swap-safe)
    def grab(sid_):
        return [dict(t) for t in db.execute(
            "SELECT player_id, throw_number, outcome, points FROM throws"
            " WHERE set_id=?", (sid_,)).fetchall()]
    src_throws, dst_throws = grab(set_id), grab(target_id)
    db.execute("DELETE FROM throws WHERE set_id IN (?, ?)",
               (set_id, target_id))
    db.execute("UPDATE sets SET home_player_id=?, away_player_id=? WHERE id=?",
               (src_["home_player_id"], src_["away_player_id"], target_id))
    db.execute("UPDATE sets SET home_player_id=?, away_player_id=? WHERE id=?",
               (dst["home_player_id"], dst["away_player_id"], set_id))
    for sid_, throws in ((target_id, src_throws), (set_id, dst_throws)):
        for t in throws:
            db.execute(
                """INSERT INTO throws (set_id, player_id, throw_number,
                     outcome, points) VALUES (?,?,?,?,?)""",
                (sid_, t["player_id"], t["throw_number"], t["outcome"],
                 t["points"]))
    ach.recompute(db, src_["season_id"])
    db.commit()
    return jsonify({"ok": True, "swapped": bool(dst_throws)})
