...
- **Projector mode** — a per-season big-type display page (Projector tab in the season nav; the top-bar shortcut jumps to the season with the most recent scoring) for a venue screen showing up to three matches at once, automatically featuring the three with the most recent scoring (a finished match stays up with a FINAL banner until a newer match replaces it): match score, per-game totals, and the live set with throw-by-throw chips, refreshing every 3 seconds, with current standings for the season being played shown underneath
