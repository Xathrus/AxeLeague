...
# --- set reset, set move, achievement collision regression ---
c.post("/seasons", data={"name": "SetOps Season"})
sidO = q("SELECT id FROM seasons ORDER BY id DESC LIMIT 1")[0]["id"]
for t in ("Reds", "Blues"):
    c.post(f"/season/{sidO}/teams", data={"name": t})
tidO = {r["name"]: r["id"] for r in q(
    "SELECT id, name FROM teams WHERE season_id=?", sidO)}
c.post(f"/team/{tidO['Reds']}/players", data={"name": "R1"})
c.post(f"/team/{tidO['Blues']}/players", data={"name": "B1"})
pidO = {r["name"]: r["id"] for r in q(
    "SELECT p.id, p.name FROM players p JOIN teams t ON p.team_id=t.id"
    " WHERE t.season_id=?", sidO)}
c.post(f"/season/{sidO}/schedule/generate")
om = q("SELECT id FROM matches WHERE season_id=? LIMIT 1", sidO)[0]["id"]
ost = state(om)
rh = pidO["R1"] if ost["match"]["home_team_id"] == tidO["Reds"] else pidO["B1"]
ra = pidO["B1"] if rh == pidO["R1"] else pidO["R1"]
osets = sets_of(om)
s_a, s_b, s_c = osets[0]["id"], osets[1]["id"], osets[3]["id"]

# both throwers score 50+ in the SAME set -> both must earn Club 50 (bug fix)
assign(s_a, rh, ra)
fill(s_a, rh, ra, ["5"]*10, ["B","5","5","5","5","5","5","5","5","5"])  # 50 & 51
both = q("""SELECT COUNT(DISTINCT player_id) n FROM achievements
            WHERE season_id=? AND key='club_50'""", sidO)[0]["n"]
ok(both == 2, "both throwers in one set earn Club 50 (collision regression)")

# set move to an empty slot
assign(s_b, rh, ra)
throw(s_b, rh, "3")
r = post_json(f"/api/set/{s_b}/move", {"target_set_id": s_c})
ok(r.status_code == 200 and r.get_json()["swapped"] is False,
   "scored set moved to an empty slot")
stO = state(om)
ok(len(stO["games"][1]["sets"][0]["home_throws"]) == 1
   and stO["games"][1]["sets"][0]["home_player_id"] == rh,
   "throws and throwers arrived at the target slot")
ok(len(stO["games"][0]["sets"][1]["home_throws"]) == 0
   and stO["games"][0]["sets"][1]["home_player_id"] is None,
   "source slot left empty")

# swap two scored sets
r = post_json(f"/api/set/{s_a}/move", {"target_set_id": s_c})
ok(r.status_code == 200 and r.get_json()["swapped"] is True,
   "two scored sets swapped")
stO = state(om)
ok(stO["games"][1]["sets"][0]["home_total"] == 50
   and stO["games"][0]["sets"][0]["home_total"] == 3,
   "swap exchanged both sets' contents")

# guardrails
om2 = q("SELECT id FROM matches WHERE season_id=? AND id != ? LIMIT 1",
        sidO, om)[0]["id"]
foreign = sets_of(om2)[0]["id"]
ok(post_json(f"/api/set/{s_a}/move",
             {"target_set_id": foreign}).status_code == 400,
   "cross-match move rejected")
ok(post_json(f"/api/set/{s_a}/move",
             {"target_set_id": s_a}).status_code == 400,
   "move to itself rejected")

# set reset
r = post_json(f"/api/set/{s_c}/reset")
ok(r.status_code == 200, "admin set reset accepted")
stO = state(om)
ok(len(stO["games"][1]["sets"][0]["home_throws"]) == 0
   and stO["games"][1]["sets"][0]["home_player_id"] is None,
   "set reset cleared throws and assignments")
ok(stO["games"][0]["sets"][0]["home_total"] == 3,
   "other sets untouched by the reset")
nach = q("""SELECT COUNT(*) n FROM achievements WHERE season_id=?
            AND key='club_50'""", sidO)[0]["n"]
...
for gi in (0, 1):
    for si in range(3):
        sx = sets_of(om)[gi*3+si]["id"]
        assign(sx, hp_, ap_)
        stq = state(om)
        hth = len(stq["games"][gi]["sets"][si]["home_throws"])
        ath = len(stq["games"][gi]["sets"][si]["away_throws"])
        for o in (["1"] * (10 - hth) if si == 0 else ["M"] * (10 - hth)):
            throw(sx, hp_, o)
        for o in ["M"] * (10 - ath):
            throw(sx, ap_, o)
post_json(f"/api/match/{om}/complete")
ok(post_json(f"/api/set/{s_a}/reset").status_code == 400,
   "completed match blocks set reset")
ok(post_json(f"/api/set/{s_a}/move",
